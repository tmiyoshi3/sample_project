...
F-04-02で確認: サプライヤー詳細画面の複数タブ（製品・契約・認証）がタブ内で直接CRUDを行う。一覧→詳細→編集の画面遷移型や管理画面（インラインCRUD）とは異なり、親エンティティの詳細画面内でサブリソースのCRUDをモーダルベースで完結させるパターン。

- タブ内の「+ 新規xxx」ボタンでモーダルを開き、作成/編集フォームを表示する
- テーブル/カード各行に「編集」「削除」ボタンを配置。編集はモーダル、削除はapp-confirm-dialogを経由
- CRUDのAPIはサブリソースAPI（`/api/{parent}/{id}/{sub-resource}`）を使用
- 作成/編集でモーダルを共用し、modeシグナル（'create' | 'edit'）で表示テキスト切替（管理画面インラインCRUDと同一ルール）
- CRUD成功後はタブ内のデータを再取得（サブリソースAPIの再呼び出し）
- モーダルのフォーム定義はReactive Formsで親コンポーネントに配置（サブコンポーネント分割は不要。フォーム項目が少ないため）

**タブ種別の使い分け:**

| タブ種別 | 内容 | 適用例 |
|---|---|---|
| 表示専用タブ | 親APIのデータまたはサブリソースAPIのデータを読み取り表示 | F-03-02（基本情報・仕様・画像・代替品・変更履歴）、F-04-02（基本情報） |
| CRUD操作タブ | サブリソースに対する作成・編集・削除操作をモーダルで提供 | F-04-02（契約タブ・認証タブ）、F-04-02（製品タブ: 追加・削除） |
| ハイブリッドタブ | 表示 + 限定的な追加操作 | F-04-02（評価履歴タブ: 一覧表示 + 新規評価追加のみ、編集・削除なし） |
...
F-04-02で確認: サプライヤー評価タブ（F-04-02-009）で、4カテゴリ（品質・納期・価格・対応）のスコアを入力・可視化する。入力にはレンジスライダー（min=1, max=5, step=0.5）、可視化には水平スコアバー（幅 = value * 20%）を使用する。

**スコア入力方式（レンジスライダー）:**
- HTML5 input[type=range] をベースにAngularバインディングで実装
- 共通コンポーネント（app-range-slider）でUI統一:
  - Input: min, max, step, value（FormControl対応）, label, showValue（数値表示、デフォルトtrue）
  - Output: valueChange
  - 現在値をスライダー横に数値表示（例: 「4.5」）
- 適用箇所: F-04-02（評価モーダルの4カテゴリスライダー）、その他のスコアリング入力が必要な画面

**スコア可視化方式（水平スコアバー）:**
- 共通コンポーネント（app-score-bar）で多次元スコアの可視化を統一:
  - Input: label（カテゴリ名）, value（0.0〜5.0）, maxValue（デフォルト5.0）, showValue（数値表示、デフォルトtrue）
  - 表示: 水平バー（幅 = value / maxValue * 100%）+ カテゴリ名 + 数値
  - バーの色はデフォルト統一色。カスタムカラー指定はオプション
- 適用箇所: F-04-02（評価エントリの品質/納期/価格/対応バー）、F-09-03（サプライヤー実績レポート）

**星評価（app-star-rating）との使い分け:**

| 表示方式 | 用途 | 適用例 |
|---|---|---|
| 星評価（app-star-rating） | 総合評価の表示（単一スコア、一覧・ヘッダ） | F-04-01（一覧テーブル）、F-04-02（ヘッダ・評価サマリー） |
| スコアバー（app-score-bar） | カテゴリ別内訳の可視化（複数スコアの比較） | F-04-02（評価エントリの4カテゴリ） |
| レンジスライダー（app-range-slider） | スコアの入力 | F-04-02（評価モーダル） |
...
- **所有権チェック**: サブリソースの更新・削除時に、そのサブリソースが指定された親リソースに属するか検証する。不一致の場合は404を返す。F-04-02で確認: 契約（contracts）・認証（certifications）・取扱製品（products）の更新・削除で所有権チェックを実施
- **CRUD対応サブリソース**: サブリソースが完全なCRUD操作（GET一覧/POST作成/PUT更新/DELETE削除）をサポートする場合、各エンドポイントの命名は標準RESTfulパターンに従う:
  - `GET /api/{resource}/{id}/{sub-resource}` — 一覧取得
  - `POST /api/{resource}/{id}/{sub-resource}` — 作成
  - `PUT /api/{resource}/{id}/{sub-resource}/{subId}` — 更新
  - `DELETE /api/{resource}/{id}/{sub-resource}/{subId}` — 削除
...
| app-range-slider | レンジスライダー入力（min/max/step設定可、数値表示、FormControl対応） | F-04-02 | 低 |
| app-score-bar | 水平スコアバー（0.0〜5.0、幅=value/max*100%、ラベル+数値表示） | F-04-02, F-09-03 | 低 |
...
| タブ内CRUD方式 | 画面遷移型 / タブ内モーダルCRUD | タブ内モーダルCRUD | F-04-02で確認。サプライヤー詳細の契約・認証・製品タブで、サブリソースのCRUDを親詳細画面のタブ内でモーダルベースで完結。管理画面インラインCRUDとは異なり、親エンティティのコンテキスト内でサブリソースを管理する |
| スコア入力方式 | テキスト入力 / セレクトボックス / レンジスライダー | レンジスライダー | F-04-02で確認。1.0〜5.0の連続値をstep=0.5で入力。スライダーは直感的で入力ミスが少なく、テキスト入力よりバリデーション負荷が低い |
| スコア可視化方式 | 数値のみ / 星表示 / 水平バー | 用途で使い分け | F-04-02で確認。総合スコアは星表示（app-star-rating）、カテゴリ別内訳は水平バー（app-score-bar）。一覧テーブルではスペース効率の良い星表示、詳細画面では比較しやすい水平バーを使用 |
| サブリソース所有権チェック | なし / APIレイヤで検証 | APIレイヤで検証 | F-04-02で確認。契約/認証/取扱製品の更新・削除時に、サブリソースが指定されたサプライヤーに属するか検証。不一致時は404を返す。RESTful設計のセキュリティベストプラクティス |
...
- サプライヤー詳細画面のロール制限（F-04-02で確認: 契約・認証・製品の全CRUD操作に@RolesAllowed未設定。CRUD操作はADMIN/MANAGER/BUYERに制限し、VIEWERは参照のみが妥当）
- サプライヤー評価の編集・削除機能の要否（F-04-02で確認: 評価は追加のみでUIに編集・削除ボタンが存在しない。移行後も追加のみとするか、管理者向けに修正・削除機能を追加するか）
- サプライヤー詳細のevaluatedBy表示改善（F-04-02で確認: ratedByフィールドがユーザーID番号のみ。移行後はUserProfile.displayNameに解決して表示する必要がある）
- サプライヤー詳細の全タブ同時ロード問題（F-04-02で確認: ngOnInit()で全5タブのAPIを同時呼び出し。移行後はタブ選択時のlazy loadに変更する方針を適用）
