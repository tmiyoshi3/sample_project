...
export class MigratedDashboardPageAdapter implements DashboardPageAdapter {
  ...
  async isPlaceholderNoteVisible(): Promise<boolean> {
    return await this.page.locator('.chart-note').first().isVisible();
  }
  ...
  async getInventoryAlertProductNames(): Promise<string[]> {
    const items = this.page.locator('.stock-alerts .alert-item .alert-product-name');
    return await items.allTextContents();
  }

  async getInventoryAlertProductSkus(): Promise<string[]> {
    const items = this.page.locator('.stock-alerts .alert-item .alert-product-sku');
    return await items.allTextContents();
  }
