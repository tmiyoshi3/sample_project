...
public class ProductRepository implements PanacheRepository<Product> {
    ...
    private static final Set<String> ALLOWED_SORT_COLUMNS = Set.of(
            "p.sku", "p.name", "c.name", "m.name", "p.unitPrice", "p.status"
    );
    ...
    public ProductSearchResult search(String keyword, Long categoryId, Long manufacturerId,
                                       String status, String sortColumn, String sortDirection,
                                       int page, int size, EntityManager em) {
        StringBuilder jpql = new StringBuilder(
                "SELECT p, c.name, m.name, COALESCE(SUM(ii.quantityOnHand), 0), u.name " +
                "FROM Product p " +
        ...
        if (keyword != null && !keyword.isBlank()) {
            where.append(" AND (LOWER(p.name) LIKE ?").append(paramIndex)
                 .append(" OR LOWER(p.description) LIKE ?").append(paramIndex)
                 .append(" OR LOWER(p.sku) LIKE ?").append(paramIndex).append(")");
