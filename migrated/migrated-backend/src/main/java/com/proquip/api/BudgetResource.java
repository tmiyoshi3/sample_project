...
public class BudgetResource {
    ...
    public List<BudgetDto> list(
            @QueryParam("fiscalYear") @DefaultValue("2026") int fiscalYear) {
        try {
            List<Budget> budgets = em.createQuery(
                    "SELECT b FROM Budget b WHERE b.fiscalYear = :fiscalYear ORDER BY b.name",
                    Budget.class)
