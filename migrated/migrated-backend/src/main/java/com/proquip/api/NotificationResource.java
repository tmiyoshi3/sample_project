...
public class NotificationResource {
	...
	private UserProfile resolveUserProfile(String principalName) {
		...
		try {
			return em.createQuery("SELECT u FROM UserProfile u WHERE u.keycloakId = :keycloakId", UserProfile.class)
					.setParameter("keycloakId", principalName).getSingleResult();
		...
		// This handles cases where the Keycloak preferred_username differs from
		// keycloakId
		try {
			return em
					.createQuery("SELECT u FROM UserProfile u WHERE u.employeeNumber = :name OR u.email = :email",
							UserProfile.class)
					.setParameter("name", principalName).setParameter("email", principalName).setMaxResults(1)
