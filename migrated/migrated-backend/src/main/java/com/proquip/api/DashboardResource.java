...
public class DashboardResource {
	...
	public DashboardSummaryDto getSummary() {
		...
		try {
			long totalProducts = em.createQuery("SELECT COUNT(p) FROM Product p WHERE p.status = 'ACTIVE'", Long.class)
					.getSingleResult();
			dto.setTotalProducts(totalProducts);

			long activeSuppliers = em
					.createQuery("SELECT COUNT(s) FROM Supplier s WHERE s.status = 'ACTIVE'", Long.class)
					.getSingleResult();
			dto.setActiveSuppliers(activeSuppliers);

			long pendingOrders = em
					.createQuery("SELECT COUNT(o) FROM PurchaseOrder o WHERE o.status = 'SUBMITTED'", Long.class)
					.getSingleResult();
			...
			long pendingRequisitions = em
					.createQuery("SELECT COUNT(r) FROM PurchaseRequisition r WHERE r.status = 'SUBMITTED'", Long.class)
					.getSingleResult();
	...
	public List<SpendingTrendItem> getSpendingTrend(@QueryParam("months") @DefaultValue("12") int months) {
		try {
			List<Object[]> results = em
					.createNativeQuery("SELECT TO_CHAR(o.order_date, 'YYYY-MM') as month, SUM(o.total_amount) as total "
	...
	public List<CategorySpendingItem> getCategorySpending() {
		try {
			List<Object[]> results = em.createQuery("SELECT c.name, COUNT(DISTINCT o.id), SUM(o.totalAmount) "
					+ "FROM PurchaseOrder o JOIN o.items oi JOIN oi.product p JOIN p.category c "
					+ "WHERE o.status NOT IN ('CANCELLED', 'DRAFT') "
					+ "GROUP BY c.name ORDER BY SUM(o.totalAmount) DESC").getResultList();
