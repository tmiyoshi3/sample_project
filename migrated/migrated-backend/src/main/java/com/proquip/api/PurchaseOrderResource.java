...
public class PurchaseOrderResource {
	...
	public PageResult<PurchaseOrderResponse> list(@QueryParam("page") @DefaultValue("0") int page,
			@QueryParam("size") @DefaultValue("100") int size, @QueryParam("status") String status,
			@QueryParam("keyword") String keyword, @QueryParam("supplierId") Long supplierId) {
		try {
			StringBuilder jpql = new StringBuilder("SELECT DISTINCT po FROM PurchaseOrder po "
					+ "LEFT JOIN FETCH po.supplier LEFT JOIN FETCH po.items");
			StringBuilder countJpql = new StringBuilder("SELECT COUNT(DISTINCT po) FROM PurchaseOrder po");
			...
			List<PurchaseOrderResponse> content = orders.stream().map(this::toResponse).collect(Collectors.toList());
	...
	private PurchaseOrderResponse toResponse(PurchaseOrder po) {
		...
		if (po.getItems() != null) {
			List<PurchaseOrderItemResponse> itemResponses = po.getItems().stream().map(this::toItemResponse)
