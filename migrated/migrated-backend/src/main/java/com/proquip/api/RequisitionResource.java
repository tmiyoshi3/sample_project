...
public class RequisitionResource {
    ...
    @GET
    public PageResult<PurchaseRequisitionDto> list(
        ...
        try {
            ...
            List<PurchaseRequisitionDto> content = requisitions.stream()
                    .map(this::toDto)
            ...
            for (PurchaseRequisitionItem item : req.getItems()) {
                if (item.getEstimatedUnitCost() != null && item.getQuantity() != null) {
                    totalAmount = totalAmount.add(
                            item.getEstimatedUnitCost().multiply(BigDecimal.valueOf(item.getQuantity())));
                }
            }
        }
