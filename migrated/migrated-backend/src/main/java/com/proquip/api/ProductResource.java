...
public class ProductResource {
	...
	public PageResult<ProductResponse> list(@QueryParam("page") @DefaultValue("0") int page,
			@QueryParam("size") @DefaultValue("20") int size, @QueryParam("keyword") String keyword,
			@QueryParam("categoryId") Long categoryId, @QueryParam("manufacturerId") Long manufacturerId,
			@QueryParam("status") String status, @QueryParam("sort") String sort) {
		...
		return productService.searchProducts(keyword, categoryId, manufacturerId, status, sortColumn, sortDirection,
				page, size);
	}
	...
	public Response createProduct(CreateProductRequest request) {
		...
		return Response.status(Response.Status.CREATED).entity(Map.of("id", product.getId())).build();
	}
	...
	public Response uploadImage(@PathParam("id") Long id, @RestForm("file") FileUpload file,
			@RestForm("fileName") String fileName, @RestForm("isPrimary") String isPrimary) {
		try {
	...
	@PermitAll
	@Consumes(MediaType.MULTIPART_FORM_DATA)
	public Response uploadDocument(@PathParam("id") Long id, @RestForm("file") FileUpload file,
			@RestForm("fileName") String fileName, @RestForm("docType") String docType) {
		try {
	...
	@RolesAllowed({"ADMIN", "MANAGER", "BUYER", "WAREHOUSE_STAFF", "VIEWER"})
	@Produces("text/csv")
	public Response exportCsv(@QueryParam("keyword") String keyword, @QueryParam("categoryId") Long categoryId,
			@QueryParam("manufacturerId") Long manufacturerId, @QueryParam("status") String status) {
		...
		return Response.ok(csv).header("Content-Disposition", "attachment; filename=\"" + filename + "\"")
				.header("Content-Type", "text/csv; charset=UTF-8").build();
	}
